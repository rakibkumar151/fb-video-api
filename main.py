...
# মাস্টার API কি (API_KEY) Hugging Face "Secrets" থেকে আসবে, তাই কোড পাবলিক থাকলেও কি কেউ জানবে না।
MASTER_API_KEY = os.environ.get("API_KEY", "")

# টেম্পোরারি টোকেন স্টোর করার জন্য
temp_tokens: Dict[str, float] = {}

def clean_expired_tokens():
    current_time = time.time()
    expired = [k for k, v in temp_tokens.items() if v < current_time]
    for k in expired:
        del temp_tokens[k]
# ------------------------------
...
@app.get("/merge")
async def merge_audio_video(video_url: str, audio_url: str, background_tasks: BackgroundTasks, title: str = "video", token: str = ""):
    # Security Check for Merge URL
    if MASTER_API_KEY:
        clean_expired_tokens()
        if token not in temp_tokens:
            raise HTTPException(status_code=401, detail="Unauthorized: Invalid or expired token. Link can only be used once.")
        # টোকেন ব্যবহার হয়ে গেলে ডিলিট করে দেওয়া হলো (One-Time Link)
    ...
    try:
        # সরাসরি FFmpeg দিয়ে URL থেকে স্ট্রিম করে ফাইল সেভ করা হবে - ডাউনলোড ওয়েট করার দরকার নেই!
        cmd = [
            "ffmpeg", "-y", 
            "-i", video_url, 
            "-i", audio_url,
...
@app.get("/get_video")
async def get_video_info(request: Request, url: str, api_key: str = "", method: str = "normal"):
    # Security Check: Master API Key
    if MASTER_API_KEY and api_key != MASTER_API_KEY:
        raise HTTPException(status_code=401, detail="Unauthorized: Invalid API Key. Please provide valid ?api_key=...")
    ...
    try:
        ...
        async def fetch_size_async(direct_url):
            try:
                head_req = await asyncio.to_thread(requests.head, direct_url, allow_redirects=True, timeout=2)
                if 'Content-Length' in head_req.headers:
                    return int(head_req.headers['Content-Length'])
            except:
                pass
            return None
        ...
        for f in formats:
            ...
            if not raw_size and direct_url and method != "fast":
                raw_size = await fetch_size_async(direct_url)
            ...
            if f_type == "Video Only (No Sound)" and best_audio_url:
                ...
                # Generate a temporary Single-Use Token for the merge URL
                clean_expired_tokens()
                merge_token = secrets.token_hex(16)
                temp_tokens[merge_token] = time.time() + 3600  # 1 hour validity before automatic deletion
                
                if MASTER_API_KEY:
                    merge_url = f"{base_url}merge?video_url={quote(direct_url)}&audio_url={quote(best_audio_url)}&title={title_param}&token={merge_token}"
                else:
